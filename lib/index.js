...
module.exports = {
  init(config) {
    ...
    const removeQueryParam = (link, paramName) =>
    {
      ...
      return params.toString() ? `${url.origin}${url.pathname}?${params.toString()}` : `${url.origin}${url.pathname}`;
    }
    ...
    const getConfigs = async () =>
    {
      ...
      return await pluginStore.get({key:'options'});
    }
    ...
    const getFilerobotUploader = (base64, file, configs) =>
    {
      const uploadImage = async () => {
        ...
        let response = await fetch(`${filerobotApiDomain}/${configs.token}/v5/files?folder=/${configs.folder}`, uploadRequestOptions);
    ...
    const getSass = async (configs) =>
    {
      ...
      if (typeof(Storage) !== "undefined" && sessionStorage.getItem("sassKey")) {
        ...
        let response = await fetch(`${filerobotApiDomain}/${configs.token}/v5/files/`, requestOptions);
        let data = await response.json();
        ...
        if (data.code !== 'KEY_EXPIRED' && data.code !== 'UNAUTHORIZED') {
          ...
          return await getNewSass(configs, requestOptions);
        }
      } else {
        // Need first Sass Key
        return await getNewSass(configs, requestOptions);
    ...
    return {
      async uploadStream(file) 
      {
        ...
        if (file.caption) {
          ...
          filerobotConfigs.sass = await getSass(filerobotConfigs);
      ...
      async delete(file) 
      {
        ...
        if (file.id) {
          ...
          let response = await fetch(`${filerobotApiDomain}/${filerobotConfigs.token}/v5/files/${file.alternativeText}`, deleteOptions);
